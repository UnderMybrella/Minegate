...
public class BlockDHD extends BlockContainer implements ILexiconable{
	...
	public void handleButtonPress(World world, int x, int y, int z, EntityPlayer player){
		TileEntityDHD dhd = (TileEntityDHD) world.getTileEntity(x, y, z);
		try{
			player.addChatMessage(new ChatComponentText("Attempting to dial " + dhd.pressedGlyphs));
			if(world.getTileEntity(dhd.boundX, dhd.boundY, dhd.boundZ) == null || !(world.getTileEntity(dhd.boundX, dhd.boundY, dhd.boundZ) instanceof TileEntityStargateController))
			{
				player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "Dialling failed - no bound controller"));
				return;
			}
			...
			if(otherController != null && otherController.worldObj.getTileEntity(otherController.x, otherController.y, otherController.z) instanceof TileEntityStargateController)
				((TileEntityStargateController) otherController.worldObj.getTileEntity(otherController.x, otherController.y, otherController.z)).disengage();
			int glyphLoc = 0;
