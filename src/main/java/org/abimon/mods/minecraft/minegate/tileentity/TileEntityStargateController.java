...
public class TileEntityStargateController extends TileEntity
{
	...
	public void updateEntity() {
		...
		if(openFor % 1200 == 0){ //Increase Cost
			System.out.println("It used to cost " + naquityCost);
			float tmp = naquityCost + ((int) prevCost / 2);
			prevCost = naquityCost;
			naquityCost = tmp;
			System.out.println("It now costs " + naquityCost);
			System.out.println(naquityStored);
			System.out.println("Open for " + openFor / 1200 + " 'minutes'");
		}


		//System.out.println(openFor + ":" + matterTicks);
	}
	...
	public static boolean isValidBlock(World worldObj, Block block){
		return block == MineGate.stargateFrame || block == MineGate.glyphBlock || block == MineGate.chevron || block instanceof INaquadahBlock || (block instanceof ITileEntityProvider && ((ITileEntityProvider) block).createNewTileEntity(worldObj, 0) instanceof INaquityProvider);
	}
	...
	public Location getOtherController(){
		...
		{	
			...
			{
				//System.out.println(glyphs.get(i) + ":" + glyphs);
				glyphs.remove(i);
				i = 0;
			}
		}
